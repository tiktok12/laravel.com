<li class="nav-docs"><a href="/docs">Documentation</a></li>
<li class="nav-laracasts"><a href="https://laracasts.com">Laracasts</a></li>
<li class="nav-forge"><a href="https://forge.laravel.com">Forge</a></li>
<li class="nav-api"><a href="/api/5.0">API</a></li>
<li class="dropdown community-dropdown">
	<a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false">Conference <span class="caret"></span></a>
	<ul class="dropdown-menu" role="menu">
		<li><a href="http://laracon.us" target="_blank">Laracon US</a></li>
		<li><a href="http://laracon.eu" target="_blank">Laracon EU</a></li>
	</ul>
</li>
<li class="dropdown community-dropdown">
	<a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false">Community <span class="caret"></span></a>
	<ul class="dropdown-menu" role="menu">
		<li><a href="https://github.com/laravel/laravel" target="_blank">Github</a></li>
		<li><a href="https://twitter.com/laravelphp" target="_blank">Twitter</a></li>
		<li class="divider"></li>
		<li><a href="https://laracasts.com/discuss">Laracasts Forums</a></li>
		<li><a href="http://laravel.io/forum" target="_blank">Laravel.io Forums</a></li>
		<li><a href="http://larachat.co" target="_blank">LaraChat Slack Channel</a></li>
		<li><a href="https://laravel-news.com/" target="_blank">Laravel News</a></li>
		<li class="divider"></li>
		<li><a href="https://larajobs.com/?partner=5#/" target="_blank">LaraJobs</a></li>
	</ul>
</li>
